...
export default function AttendanceResultPage() {
  ...
  useEffect(() => {
    ...
    setAttendanceData({
      ...
      name:
        toSingleString(params.name as string | string[]) ||
        toSingleString(params.username as string | string[]),
      deviceId: toSingleString(params.deviceId as string | string[]),
      deviceName: toSingleString(params.deviceName as string | string[]),
      timestamp:
        toSingleString(params.timestamp as string | string[]) ||
        new Date().toISOString(),
      location: toSingleString(params.location as string | string[]),
      status:
        toSingleString(params.status as string | string[]) === "late" ||
        toSingleString(params.status as string | string[]) === "absent"
          ? (toSingleString(params.status as string | string[]) as
              "late" | "absent")
          : "present",
    });
  }, [params]);
  ...
  const handleNavigateToAttendance = () => {
    router.navigate("/(protected)/admin/(tabs)/AttendanceTab");
  };
